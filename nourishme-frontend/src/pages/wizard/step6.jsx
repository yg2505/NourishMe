  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {dietsList.map((diet) => (
            <div
              key={diet}
              onClick={() =>
                updateForm({ dietType: diet, suggestedDiet: "" })
              }
              className={`p-4 border rounded-xl cursor-pointer text-center font-medium transition-all
                ${
                  form.dietType === diet
        ...
        <button
          onClick={suggestDiet}
          className="w-full px-5 py-2 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition"
        >
          Suggest Diet Based on BMI
        </button>

        {/* Suggested Diet Display */}
        {form.suggestedDiet && (
          <div className="text-center space-y-1">
            {bmi && (
              <p className="font-semibold text-gray-700">Your BMI: {bmi}</p>
            )}
